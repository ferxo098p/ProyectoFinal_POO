...
/**
* Clase VentanaCambioModificaciones
* Esta clase contiene el diseño y la lógica de la 
* subventana de la ventana para cambios del sistema
* @author Fernando Sánchez Escobar
* @version 1.0 
*/

public class VentanaCambioModificaciones extends JDialog implements ActionListener{
	//Atributos
	/**
	...
	*/
	...
	/**
	* Regresa a la ventana para cambios del sistema
	*/
	private JButton cancelar;
	/**
	* Confirma los cambios en caso de no presentarse algun error
	*/
	...
	*/
	...
	/**
	* Almacena los datos del automovil actualizado
	*/
	Automovil auto = new Automovil();
	/**
	* Indica si se ha confirmado la accion
	*/
	...
	*/
	...
	//Constructor
	/**
	* Constructor con dos párametros, crea una instancia de VentanaCambioModificaciones
	* @param ventana ventana desde la cual fue invocada
	* @param modal indica si bloqueara la ventana que la invoco 
	*/
	public VentanaCambioModificaciones(JFrame ventana,boolean modal){
		super(ventana,modal);	
		this.crearVentanaCambioModificaciones();
	...
	*/
	public void crearVentanaCambioModificaciones(){
		//Propiedades de la ventana de modificaciones de cambio
		...
		//Con esto se indica que no se pueda cerrar la ventana por medio de X
		this.setDefaultCloseOperation(WindowConstants.DO_NOTHING_ON_CLOSE);
		//this.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
		this.setLocationRelativeTo(null);
	}
	/**
	* Crea y agrega los componentes que tendrá la ventana de modificaciones de cambio
	*/
	public void agregarComponentes(){
		JLabel etiqueta1 = new JLabel("Sistema de Valet Parking - Modificaciones");
		...
		etiqueta1.setBounds(140,20,350,30);
	...
	* @see unam.fi.poo.graficos.VentanaMenu
	*/
			...
			if(estado == false){
				...
				if(!info.getEstacionamiento().verificarEspacio()){
					if(info.getEstacionamiento().buscarAuto(cajaMatriculaD.getText()) == true){
						...
						JOptionPane.showMessageDialog(this, "Modificaciones guardadas!!!");
						//this.actualizarAutomovil(info);
	...
	/**
	* Agrega las validaciones que tendra la subventana
	...
	* @see unam.fi.poo.validaciones.ValidacionVentanaCambio
	*/
	public void verificarCampos(){
		ValidacionVentanaCambio.verificarCajaMarca(cajaMarca);
		ValidacionVentanaCambio.verificarCajaModelo(cajaModelo);
		ValidacionVentanaCambio.verificarCajaColor(cajaColor);
		ValidacionVentanaCambio.verificarCajaMatriculaD(cajaMatriculaD);
		ValidacionVentanaCambio.verificarCajaMatriculaT(cajaMatriculaT);
		ValidacionVentanaCambio.verificarCajaCapacidad(cajaCapacidad);
		ValidacionVentanaCambio.verificarCajaNombre(cajaNombre);
		ValidacionVentanaCambio.verificarCajaApellidoP(cajaApellidoP);
		ValidacionVentanaCambio.verificarCajaApellidoM(cajaApellidoM);
		ValidacionVentanaCambio.verificarCajaEdad(cajaEdad);
		ValidacionVentanaCambio.verificarCajaEstancia(cajaEstancia);
	}
	/**
	* Actualiza la información de un automovil
	* @param info nos proporciona información sobre el estacionamiento
	* @see unam.fi.poo.graficos.VentanaCambio
	* @see unam.fi.poo.clases.Propietario
	*/
	public void actualizarAutomovil(Informador info){
		String md = VentanaCambio.filaActualizar.get(3);
		auto = info.getEstacionamiento().entregarAuto(md);
		...
		propietario.setNombre(cajaNombre.getText());
		propietario.setApellidoP(cajaApellidoP.getText());
		propietario.setApellidoM(cajaApellidoM.getText());
		propietario.setEdad(Integer.parseInt(cajaEdad.getText()));
		auto.setPropietario(propietario);
		info.getEstacionamiento().agregarAuto(auto);		
	}
	/**
	* Haciendo uso de la fila a actualizar de la ventana de cambios
	* coloca la información que esta tiene en el correspondiente 
	* campo de texto
	* @see unam.fi.poo.graficos.VentanaCambio
	*/
	public void llenarCampos(){
		cajaMarca.setText(VentanaCambio.filaActualizar.get(0));
		cajaModelo.setText(VentanaCambio.filaActualizar.get(1));
		cajaColor.setText(VentanaCambio.filaActualizar.get(2));
		cajaMatriculaD.setText(VentanaCambio.filaActualizar.get(3));
		cajaMatriculaT.setText(VentanaCambio.filaActualizar.get(4));
		cajaCapacidad.setText(VentanaCambio.filaActualizar.get(5));
		cajaEstancia.setText(VentanaCambio.filaActualizar.get(7));
		cajaNombre.setText(VentanaCambio.filaActualizar.get(8));
		cajaApellidoP.setText(VentanaCambio.filaActualizar.get(9));
		cajaApellidoM.setText(VentanaCambio.filaActualizar.get(10));
		cajaEdad.setText(VentanaCambio.filaActualizar.get(11));
	}
	/**
	* Crea y grega una fila con la informacion actualizada al modelo que tiene 
	* la JTable 
	* @param tabla JTable que contiene el listado de automoviles registrados
	* @param modelo modelo que tiene la tabla
	*/
	public void agregarFila(JTable tabla,DefaultTableModel modelo){
		String[] fila = new String[12];
		this.llenarFila(fila);
		modelo.addRow(fila);
	}
	/**
	* Llena la fila con la información actualizada
	* @param fila arreglo de String que contendrá la información actualizada
	*/
	public void llenarFila(String[] fila){
		int tiempo = Integer.parseInt(cajaEstancia.getText());
		auto.setCapacidad(Integer.parseInt(cajaCapacidad.getText()));
		auto.calcularTarifa(tiempo);
		fila[0] = cajaMarca.getText();
		fila[1] = cajaModelo.getText();
		fila[2] = cajaColor.getText();
		fila[3] = cajaMatriculaD.getText();
		fila[4] = cajaMatriculaT.getText();
		fila[5] = cajaCapacidad.getText();
		fila[6] = String.valueOf(auto.getTarifa());
		fila[7] = String.valueOf(cajaEstancia.getText());
		fila[8] = cajaNombre.getText();
		fila[9] = cajaApellidoP.getText();
		fila[10] = cajaApellidoM.getText();
		fila[11] = cajaEdad.getText();
	}
	/**
	* Indica si se ha confirmado la actualización
	* @return <ul>
	*	<li> true: se confirmo la actualización </li>
	*	<li> false: no se confirmo la actualización </li>
	*	</ul>
	*/
